use crate::utils::seeder;
use wasm_bindgen::prelude::*;

#[wasm_bindgen]
pub fn manga_title() -> String {
	TITLES[seeder::gen_range(0..TITLES_LEN)].to_string()
}

#[wasm_bindgen]
pub fn manga_genre() -> String {
	GENRES[seeder::gen_range(0..GENRES_LEN)].to_string()
}

#[wasm_bindgen]
pub fn manga_platform() -> String {
	DEMOGRAPHICS[seeder::gen_range(0..DEMOGRAPHICS_LEN)].to_string()
}

static TITLES: [&'static str; 500] = [
	"Berserk",
	"Monster",
	"Nausicaä of the Valley of the Wind",
	"Steel Ball Run",
	"Vinland Saga",
	"Yokohama Kaidashi Kikou",
	"20th Century Boys",
	"Fullmetal Alchemist",
	"Akira",
	"Yotsuba&!",
	"Vagabond",
	"GTO",
	"REAL",
	"Phoenix",
	"Death Note",
	"With the Light: Raising an Autistic Child",
	"Mushishi",
	"Pluto",
	"Goodnight Punpun",
	"Hajime no Ippo",
	"Mobile Suit Gundam: The Origin",
	"Cross Game",
	"Fruits Basket",
	"Buddha",
	"A Brides Story",
	"Skip Beat!",
	"Lone Wolf and Cub, Kazuo Koike",
	"Rurouni Kenshin",
	"A Drifting Life",
	"Sanctuary by R. Ikegami",
	"JoJolion",
	"A Silent Voice",
	"Glass no Kamen",
	"Showa: A History of Japan",
	"Blade of the Immortal",
	"Ōoku: The Inner Chambers",
	"Solanin",
	"Message to Adolf",
	"Town of Evening Calm, Country of Cherry Blossoms",
	"Spirit Circle",
	"Kingdom",
	"Erased",
	"Saikano",
	"Kids on the Slope",
	"Genshiken",
	"Frieren: Beyond Journeys End",
	"Azumanga Daioh",
	"Battle Angel Alita",
	"Maison Ikkoku",
	"Ode to Kirihito",
	"Ashita no Joe",
	"Historie",
	"(The) Heart of Thomas",
	"Stigma",
	"One-Punch Man",
	"Planetes",
	"Kaguya-sama: Love is War",
	"Domu",
	"Angel Densetsu",
	"One Piece",
	"My Lesbian Experience with Loneliness",
	"21st Century Boys",
	"NANA",
	"Black Jack",
	"Kurosawa the Master of Masturbation",
	"Hunter × Hunter",
	"Wandering Son",
	"Grand Blue Dreaming",
	"Bakuman.",
	"Dengeki Daisy",
	"Hana to Akuma",
	"Nodame Cantabile",
	"Ludwig Kakumei",
	"Lucifer and the Biscuit Hammer",
	"Beck",
	"(The) World God Only Knows",
	"Twin Spica",
	"Girls Last Tour",
	"Shigurui",
	"Girlfriends",
	"Yona of the Dawn",
	"Saiyuki Gaiden",
	"Ouran High School Host Club",
	"(The) Flowers of Evil",
	"Children of the Sea",
	"Ookiku Furikabutte",
	"Claymore",
	"(The) World is Mine",
	"Shijō Saikyō no Deshi Kenichi",
	"Slam Dunk",
	"Me and the Devil Blues",
	"Ultra Heaven",
	"A Town Where You Live",
	"Pandora Hearts",
	"Space Brothers",
	"Akane-banashi",
	"Misaki de Bus o Orita Hito",
	"Gunslinger Girl",
	"Cardcaptor Sakura: Master of the Clow",
	"Bokurano: Ours",
	"H2",
	"Kanojo no Omoide...",
	"A Drunken Dream",
	"Your Lie in April",
	"Tokyo Crazy Paradise",
	"Full Metal Panic! Sigma",
	"NonNonBâ",
	"Hells Paradise: Jigokuraku",
	"Blue Flag",
	"Blank Canvas: My So-Called Artists Journey",
	"Tokyo Babylon",
	"Dandadan",
	"(The) Worlds Greatest First Love: The Case of Ritsu Onodera",
	"Rough",
	"Attack on Titan",
	"7SEEDS",
	"Koori no Mamono no Monogatari",
	"Love*Com",
	"Emma",
	"Ao Haru Ride",
	"Ping Pong",
	"Hellsing",
	"Liar Game",
	"Wild Adapter",
	"ReLIFE",
	"Kimi ni Todoke - From Me to You",
	"Das Fenster von Orpheus",
	"Parasyte",
	"Moe Kare!!",
	"Black Butler",
	"Mars",
	"Natsumes Book of Friends",
	"Last Quarter",
	"Embracing Love",
	"Whispered Words",
	"(The) Ancient Magus Bride",
	"Sakamoto Days",
	"Iris Zero",
	"Kodocha - Sanas Stage",
	"Bitter Virgin",
	"Tanpenshu",
	"Iryū - Team Medical Dragon",
	"M×0",
	"Akkanbe Ikkyu",
	"5 Centimeters per Second",
	"Ghost in the Shell",
	"A Distant Neighborhood",
	"Strawberry Shake Sweet",
	"Princess Jellyfish",
	"Finder",
	"Paradise Kiss",
	"Gakuen Alice",
	"Dame na Watashi ni Koishite Kudasai",
	"Piano no Mori",
	"Sakura-Gari",
	"Cat Street",
	"(The) Five Star Stories",
	"Chihayafuru",
	"xxxHOLiC",
	"Pet Shop of Horrors",
	"My Love Story!!",
	"Barefoot Gen",
	"Eden - It is an Endless World!, Hiroki Endo",
	"Gantz",
	"Holyland",
	"(The) Betrayal Knows My Name",
	"Hana-Kimi",
	"Kare Kano",
	"In This Corner of the World",
	"They Were Eleven",
	"Kaze to Ki no Uta",
	"Short Program",
	"Kiichi!!",
	"Battle Angel Alita: Last Order",
	"(The) Key to the Kingdom",
	"March comes in like a lion",
	"Rosario + Vampire Season II",
	"(The) Summit of the Gods",
	"Dead Dead Demons Dededede Destruction",
	"Aria",
	"2001 Nights",
	"Samurai Executioner",
	"Billy Bat",
	"Higurashi no Naku Koroni",
	"Descending Stories: Showa Genroku Rakugo Shinju",
	"We Were There",
	"Wotakoi: Love is Hard for Otaku",
	"Dusk maiden of Amnesia",
	"Rainbow - Nisha Rokubō no Shichinin",
	"Black Lagoon",
	"A Journal of My Father",
	"Good-Bye by Y. Tatsumi",
	"(The) Voices of a Distant Star",
	"Koikaze",
	"Beyond Twilight",
	"Snow White with the Red Hair",
	"Given",
	"Akumetsu",
	"Path of the Assassin",
	"(The) 100 Girlfriends Who Really, Really, Really, Really, Really Love You",
	"Ressentiment",
	"Mahoraba",
	"MW",
	"Jujutsu Kaisen",
	"Baby Steps",
	"Spice & Wolf",
	"Delicious in Dungeon",
	"Orange",
	"Boku to Issho",
	"Only the Ring Finger Knows",
	"Complex Age",
	"Tsubasa, RESERVoir CHRoNiCLE",
	"Kimagure Orange Road",
	"Higurashi When They Cry: Cotton Drifting Arc",
	"After the Rain",
	"Junjo Romantica",
	"Gambling Apocalypse Kaiji",
	"Noragami: Stray God",
	"Basara",
	"Haikyu!!",
	"Bloom Into You",
	"Look Back",
	"Puella Magi Madoka Magica",
	"Silver Spoon",
	"Cardcaptor Sakura",
	"Eyeshield 21",
	"(The) Rose of Versailles",
	"I am a Hero",
	"Kyou Kara Ore Wa!!",
	"Count Cain",
	"Tokyo Ghoul",
	"(The) Drifting Classroom",
	"Kabu no Isaki",
	"Bad Teachers Equation",
	"Welcome to the N.H.K.",
	"Hellhounds: Panzer Cops",
	"Katsu!",
	"(The) Story of Saiunkoku",
	"Japan as Viewed by 17 Creators",
	"Higurashi When They Cry: Massacre Arc",
	"Ghost in the Shell: Stand Alone Complex",
	"Hikaru no Go",
	"Stringendo",
	"Ai-Ren",
	"Inugami",
	"Blue by K. Nananan",
	"GE - Good Ending",
	"Devilman",
	"Negima! Magister Negi Magi",
	"Chainsaw Man",
	"A Zoo in Winter",
	"(The) Garden of Words",
	"Disappearance Diary",
	"Zipang",
	"Chrono Crusade",
	"Nana to Kaoru",
	"Sekai no Owari to Yoake-Mae",
	"(The) Record of a Fallen Vampire",
	"Blame!",
	"(The) Cain Saga",
	"ES",
	"Rivers Edge",
	"Love Mode",
	"Wolf Children: Ame & Yuki",
	"Love Hina",
	"Little Butterfly",
	"Witch Hat Atelier",
	"Gangsta.",
	"Sunny",
	"Love Hina [anime comic]",
	"No Matter How I Look at It, It is You Guys Fault I am Not Popular!",
	"Neon Genesis Evangelion",
	"Kamisama Kiss",
	"Non Non Biyori",
	"Doraemon",
	"Blood Alone",
	"Parfait Tic!",
	"Takemitsuzamurai",
	"Deep Love: Ayu no Monogatari",
	"A Strange Kind of Woman",
	"Rin!",
	"Hayate X Blade",
	"Darwins Game",
	"Sasaki and Miyano",
	"Flower of Life",
	"Hitler",
	"Power Play!",
	"Vassalord",
	"Wake Up, Sleeping Beauty",
	"Boys Run the Riot",
	"Utahime: The Songstress",
	"Yankee-kun to Megane-chan",
	"Sidooh",
	"Soul Eater",
	"Beautiful People",
	"Shiki",
	"Inu × Boku SS",
	"Is",
	"Dogs: Bullets & Carnage",
	"Witches",
	"Gimmick!",
	"Higurashi When They Cry: Festival Accompanying Arc",
	"Until Death Do Us Part",
	"Yowamushi Pedal",
	"Boys Next Door",
	"FAKE",
	"Nura: Rise of the Yokai Clan",
	"Homunculus",
	"Awkward Silence",
	"Love So Life",
	"Deep Love: Pao no Monogatari",
	"(The) Day I Become a Butterfly",
	"(The) Tyrant Falls in Love",
	"Q.E.D.",
	"Tokimeki Tonight",
	"Angel Sanctuary",
	"Noramimi",
	"(The) Gods Lie",
	"Goodbye, Eri",
	"Soredemo Machi wa Mawatteiru",
	"(The) Case Study of Vanitas",
	"Kizuna - Bonds of Love",
	"Swing Shell",
	"Durarara!! Saika Arc",
	"Bokkō",
	"Chios School Road",
	"My Hero Academia: Vigilantes",
	"Shōjo x Shōjo x Shōjo",
	"Ballad of a Shinigami",
	"Blue Exorcist",
	"Higurashi When They Cry: Curse Killing Arc",
	"Land of the Lustrous",
	"Solitary Gourmet",
	"Zankokuna Kami ga Shihai suru",
	"Koi ni Ochita Ōjisama",
	"Candy Candy",
	"Please Tell Me! Galko-chan",
	"Full Moon O Sagashite",
	"Honey and Clover",
	"Gerard & Jacques",
	"Übel Blatt",
	"Monthly Girls Nozaki-kun",
	"My Hero Academia",
	"(The) Ice Wanderer",
	"(The) Promised Neverland",
	"Pokémon Adventures",
	"Puella Magi Madoka Magica: The Different Story",
	"My Solo Exchange Diary",
	"Nudist Beach ni Shūgakuryokō de!!",
	"Fujoshi Rumi: Mousou Shoujo Otaku Kei",
	"Petit Monster - Puchimon",
	"Fly Me to the Moon",
	"Sket Dance",
	"Ten Count",
	"Our Kingdom",
	"Mobile Suit Gundam Seed Destiny Astray",
	"Higurashi When They Cry: Eye Opening Arc",
	"Abandon the Old in Tokyo",
	"Kyōshi to Seito to",
	"Helen esp",
	"Not Simple",
	"(The) High School Life of a Fudanshi",
	"Seven Days: Friday-Sunday",
	"Cage of Eden",
	"JoJos Bizarre Adventure",
	"Naruto",
	"I Sold My Life for Ten Thousand Yen Per Year",
	"Yakitate!! Japan",
	"Togainu no Chi",
	"Immortal Rain",
	"Maid-sama!",
	"Tropical Citron - Psychedelic Witch Story",
	"Seven Days: Monday-Thursday",
	"GoGo Monster",
	"Elfen Lied",
	"Himizu",
	"Beast Master by K. Motomi",
	"Havent You Heard? I am Sakamoto",
	"Shin Tekken Chinmi",
	"I Hear the Sunspot",
	"Higurashi When They Cry: Time Killing Arc",
	"Shinigami-sama ni Saigo no Onegai o",
	"Boing Boing",
	"Young Miss Holmes",
	"Quest for the Missing Girl",
	"Binbō-gami ga!",
	"Suzuka",
	"School Rumble",
	"Hinamatsuri",
	"Psyren",
	"Niea_7",
	"Insomniacs After School",
	"Firefighter! Daigo of Fire Company M",
	"Daily Lives of High School Boys",
	"Appleseed",
	"Karneval",
	"Fushigi Yûgi: Genbu Kaiden",
	"Ran And The Gray World",
	"X",
	"Higurashi When They Cry: Abducted by Demons Arc",
	"D.Gray-man",
	"Hyouge Mono - Tea for Universe, Tea for Life",
	"Koizora ~Setsunai Koi Monogatari~",
	"Tekkonkinkreet",
	"No. 6",
	"Ichigeki Sacchu!! HoiHoi-san",
	"SOIL",
	"Kimi to Boku",
	"Empty Heart",
	"Kinnikuman",
	"Gokukoku no Brynhildr",
	"Getter Robo",
	"ONE OUTS - Nobody wins, but I!",
	"Dear",
	"Young Kindaichis Casebook Special",
	"From Far Away",
	"Tokyo Tarareba Girls",
	"Ai no Tame ni",
	"Dorohedoro",
	"All My Darling Daughters",
	"Dragon Eye",
	"Shikeishuu 042",
	"Natsu no Zenjitsu",
	"Kindaichi-Shonen no Jikenbo Tanpenshu",
	"Bleach",
	"Blue Period",
	"(The) Kurosagi Corpse Delivery Service",
	"(The) Music of Marie",
	"GA: Geijutsuka Art Design Class",
	"Future Diary",
	"Mirai no Kioku: Kaze no Yukue",
	"Sayonara, Zetsubou-Sensei",
	"Yuruyuri",
	"Kakuriyo: Bed & Breakfast for Spirits, Waco Ioka",
	"Help!!",
	"Only Serious About You",
	"All You Need Is Kill",
	"Higurashi When They Cry: Demon Exposing Arc",
	"Heaven!!, Shizuru Seino",
	"Essence",
	"Kinos Journey - The Beautiful World Iruka Shiomiya",
	"Sunshine Sketch",
	"Captain Harlock",
	"Eensy Weensy Monster",
	"Deep Love: Reina no Unmei",
	"Close the Last Door",
	"(The) Eminence in Shadow",
	"Switch by naked ape",
	"Say I Love You.",
	"Voice Over! Seiyu Academy",
	"Aqua",
	"Jungle Emperor Leo",
	"Under Grand Hotel",
	"Free Soul",
	"Sekirei",
	"Hayate the Combat Butler",
	"Chocolat by Kubonouchi",
	"Love My Life",
	"SPY×FAMILY",
	"Sakura Trick",
	"Hellsing: The Dawn",
	"OPUS",
	"Ratman",
	"Tsuki to Mizu no Yoru",
	"Train Man: Densha Otoko",
	"Kokou no Hito",
	"Inari, Konkon, Koi Iroha",
	"Yellow",
	"Demon Flowers: Kuruizaki no Hana",
	"Kowloon Generic Romance",
	"Gunsmith Cats",
	"Yukarism",
	"Future Lovers",
	"Emanon",
	"Partner",
	"Saiyuki Reload",
	"Hanada Shōnen-shi",
	"Level E",
	"Akagi",
	"Orenji Yane no Chiisana Ie",
	"Shocking Pink!",
	"Blue Morning",
	"Junji Itos Cat Diary: Yon & Mu",
	"Gakuen Babysitters",
	"Yesterday o Utatte",
	"LDK",
	"Love Roma",
	"Needless",
	"Interspecies Reviewers",
	"Gin Tama",
	"Hotarubi no Mori e",
	"Summer Wars",
	"Golden Kamuy",
	"NightS",
	"Chacha The Little Red Riding Hood",
	"Crazy for You",
	"Crimson Spell",
	"Dainippon Tengutou Ekotoba",
	"Tail of the Moon",
	"Strawberry 100%",
];
static TITLES_LEN: usize = TITLES.len();

static GENRES: [&'static str; 16] = [
	"Action",
	"Adventure",
	"Comedy",
	"Sports",
	"Drama",
	"Slice of Life",
	"Erotica",
	"Fantasy",
	"Horror",
	"Mystery",
	"Thriller",
	"Psychological",
	"Romance",
	"Science Fiction",
	"Yaoi",
	"Yuri",
];
static GENRES_LEN: usize = GENRES.len();

static DEMOGRAPHICS: [&'static str; 5] = [
	"Shōnen",
	"Shōjo",
	"Seinen",
	"Josei",
	"Kodomo-Muke",
];
static DEMOGRAPHICS_LEN: usize = DEMOGRAPHICS.len();
